...
pub fn resolve_project_root(command: &Commands, output: &mut Output) -> Result<(PathBuf, bool)> {
    match command {
        ...
        Commands::Pull { .. } => {
            ...
            if current_dir.join(WAYLOG_DIR).is_dir() {
                output.found_tracking(&current_dir)?;
                return Ok((current_dir, false));
            }

            // Interactive prompt for initialization
            output.not_initialized()?;
            output.init_prompt(&current_dir.join(WAYLOG_DIR))?;
        ...
        Commands::Run { .. } => match crate::utils::path::find_project_root() {
