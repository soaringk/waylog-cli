...
- **One hierarchy for both formats:** `exporter::entries` splits a session once into standalone records and assistant turns. Markdown renders `##`/`###`, JSON renders `turns`/`parts`. Neither invents a shape the other lacks.
