...
The installers select the matching GitHub Release binary for macOS, Linux, or Windows on x64 or ARM64 and verify its SHA-256 checksum. Set `WAYLOG_VERSION` (for example, `0.3.1`) before running the installer to pin a release instead of using the latest version.
...
A source directory may contain contributor subdirectories. Run the command once for each downloaded provider directory; supplied artifacts are authoritative and regenerate their Markdown.
